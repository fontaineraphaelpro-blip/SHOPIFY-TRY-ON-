...
# --- 2. FONCTIONS UTILITAIRES SHOPIFY ---
...
async def track_atc(req: dict):
    # Simplifié pour accepter JSON direct
...
# --- ROUTE GÉNÉRATION (LE COEUR DU SYSTÈME) ---
...
async def generate(
    ...
    clothing_url: Optional[str] = Form(None), # Reçoit l'URL Shopify
    category: str = Form("upper_body")
):
    print(f"📥 GEN REQUEST: Shop={shop}")
    
    # 1. Vérification Shop & Token
    shop = clean_shop_url(shop)
    token = get_token_db(shop)
    ...
    try:
        ...
        # 4. Préparation Inputs pour Replicate
        
        # A. Image Humain (Fichier Bytes)
        person_bytes = await person_image.read()
        person_file = io.BytesIO(person_bytes)
        
        # B. Image Vêtement (URL ou Fichier)
        garment_input = None
        
        if clothing_url and len(clothing_url) > 5:
            # CAS URL (Client Mode) : On envoie l'URL directement (string)
            garment_input = clothing_url.strip()
            # Sécurité : s'assurer que c'est une URL valide
            if garment_input.startswith("//"): 
                garment_input = "https:" + garment_input
            print(f"👕 Using Clothing URL: {garment_input}")
            
        elif clothing_file:
            # CAS FICHIER (Admin Mode)
            g_bytes = await clothing_file.read()
            garment_input = io.BytesIO(g_bytes)
            print("👕 Using Clothing File Upload")
        else:
        ...
        output = replicate.run(
            MODEL_ID,
            input={
                "human_img": person_file,     # Bytes
                "garm_img": garment_input,    # String (URL) ou Bytes
