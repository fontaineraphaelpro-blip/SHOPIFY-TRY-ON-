document.addEventListener("DOMContentLoaded", function() {
    ...
    async function initAdminMode(s) {
        ...
        if (res && res.ok) {
            ...
            // Affichage Stats
            updateDashboardStats(data.credits || 0, data.usage || 0);
    ...
    function updateDashboardStats(credits, usage) {
        const el = document.getElementById('credits');
        if(el) el.innerText = credits;

        const usageEl = document.getElementById('stat-tryons');
        if(usageEl) usageEl.innerText = usage;
    ...
    window.generate = async function() {
        ...
        try {
            ...
            if(data.result_image_url) {
                ...
                if(data.new_credits !== undefined || data.usage !== undefined) {
                    updateDashboardStats(data.new_credits || 0, data.usage || 0);
                }
            } else {
